...
def test_recipe_catalog_contains_core() -> None:
    ...
    assert {"cartpole-balance", "g1-walk", "g1-stand", "pick-and-place"} <= ids
...
def test_recipe_catalog_is_the_studio_contract() -> None:
    ...
    assert set(catalog["later"]) == {"g1-walk"}
...
def test_launch_here_walk_when_playground_ready(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "g1-reach" not in catalog["later"]
...
def test_recipes_pin_real_upstream_task_ids() -> None:
    ...
    # No invented reach env: the dishonest g1-reach recipe is gone (Phase 3f).
    assert not (root / "recipes" / "g1-reach").exists()
    assert not (root / "spec" / "examples" / "g1-reach.json").is_file()
